...
import { Search, Plus, Eye, Edit2, Trash2, X, Save, User as UserIcon, Mail, Phone, Shield } from 'lucide-react';
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Toast.fire({
          title: 'USER DELETED',
          text: 'Account has been removed from the registry.',
          icon: 'success',
          timer: 2000,
        ...
        Toast.fire({
          title: 'ERROR',
          text: 'Failed to terminate account: ' + error,
          icon: 'error'
        });
      }
    }
  ...
  const handleUpdate = async () => {
    try {
      ...
      Toast.fire({
        ...
        text: 'User profile intelligence has been synchronized.',
        icon: 'success',
        timer: 2000,
        showConfirmButton: false
      });
      ...
      Toast.fire({
        title: 'SYNC ERROR',
        text: 'Failed to update user intelligence: ' + error,
        icon: 'error'
      });
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium">Manage user accounts and their permissions.</p>
        </div>
        <button className="flex items-center gap-2 bg-[#c5a059] hover:bg-[#b08d4a] text-white font-black px-8 py-3.5 rounded-full text-xs uppercase tracking-widest transition-all shadow-lg shadow-[#c5a059]/10">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-[10px] text-slate-600 uppercase font-black tracking-widest border-b border-gray-100">
              <tr>
                {['User ID', 'User Details', 'Phone Number', 'Role', 'Applications', 'Joined', 'Actions'].map(h => (
              ...
              {filtered.map(u => (
                <tr key={u.id} className="hover:bg-gray-50 transition-colors group">
                  <td className="px-8 py-5 font-mono text-[10px] text-slate-500">#USR-{u.id.toString().padStart(4, '0')}</td>
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        <img src={u.avatar} alt={u.name} className="w-10 h-10 rounded-2xl object-cover ring-2 ring-[#c5a059]/30" />
                        <div className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border-2 border-white ${u.role === 'admin' ? 'bg-[#c5a059]' : 'bg-blue-500'}`} />
                      </div>
                      <div>
                  ...
                  <td className="px-6 py-4 text-slate-600 font-medium whitespace-nowrap text-xs">{u.phone || 'No Phone Sync'}</td>
                  <td className="px-8 py-5">
                    <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${
                      u.role === 'admin' ? 'bg-[#c5a059]/10 text-[#c5a059] border border-[#c5a059]/20' : 'bg-blue-500/10 text-blue-500 border border-blue-500/20'
                    }`}>
                      {u.role || 'User'}
                    </span>
                  </td>
                  ...
                  <td className="px-8 py-5 text-slate-500 text-[11px] font-black uppercase tracking-widest whitespace-nowrap">{u.joined}</td>
              ...
              <div className="p-8 space-y-6">
               <div>
                 ...
                 <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block ml-1">User Role</label>
                    <div className="relative group">
